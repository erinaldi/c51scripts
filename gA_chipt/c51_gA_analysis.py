...
try:
    ...
    print('For a help message including list of required libraries')
    print('RUN: python c51_gA_analysis.py -h')
    exit()
...
def chipt_fit(args,p,data):
    def print_output(CS,ga_min,cov_lam):
        # central value
        ga_fit = gafit.ga_epi(CS.x0,CS.xphys,0,**ga_min.values)
        ...
        return {'ga_fit':ga_fit, 'dga_fit':dga_fit}
    ...
    if args.fits in ['all','taylor_esq_1']:
        ...
        # print outputs
        result = print_output(CS,ga_min,cov_lam)
        return result
...
if __name__=='__main__':
    ...
    fig = 1
        ...
        if args.plot:
            ...
            ga_mpi_ax = plt.axes(epi_axes)
            leg1 = []
            leg2 = []
            epi_plot = np.arange(0.001,0.41,.001)
            xplot = epi_plot
            # continuum limit plot
            ga_plot = fit.ga_epi(x0,epi_plot**2,0,c0,ca2=ca2,cm1=cm1,cm2=cm2)
            dga_plot = fit.dga_epi(x0,epi_plot**2,0,c0,cov_lam[0:-1,0:-1],ca2=ca2,cm1=cm1,cm2=cm2)
            ...
            leg2.append(leg)
            # finite a plots
            ga_plot15 = fit.ga_epi(x0,epi_plot**2,aw0['a15m310'],c0,ca2=ca2,cm1=cm1,cm2=cm2)
            ga_plot12 = fit.ga_epi(x0,epi_plot**2,aw0['a12m310'],c0,ca2=ca2,cm1=cm1,cm2=cm2)
            ga_plot09 = fit.ga_epi(x0,epi_plot**2,aw0['a09m310'],c0,ca2=ca2,cm1=cm1,cm2=cm2)
            ...
            leg, = ga_mpi_ax.plot(xplot,ga_plot15,color=rd,alpha=0.5,
                label=r'$g_A(\epsilon_\pi,a=0.15)$')
            leg1.insert(0,leg)
            leg, = ga_mpi_ax.plot(xplot,ga_plot12,color=gn,alpha=0.5,
                label=r'$g_A(\epsilon_\pi,a=0.12)$')
            leg1.insert(0,leg)
            leg, = ga_mpi_ax.plot(xplot,ga_plot09,color=bl,alpha=0.5,
            ...
            for i,ens in enumerate(ensembles):
                ...
                mkr = e_mrkr[ens]
                ei = epi_b0[i]
                dei = epi_bs.std(axis=0)[i]
                dfv = fit.dgaFV(ei,mL[i],g0fv)
                gi = ga_b0[i]
                ...
                if ens in a_i:
                    leg1.insert(3,leg)
            ga_mpi_ax.set_xlabel(r'$\epsilon_\pi = m_\pi /(4\pi F_\pi)$',fontsize=fs)
            ga_mpi_ax.set_ylabel(r'$g_A$',fontsize=fs)
            leg = ga_mpi_ax.errorbar(epi_phys,ga_phys,yerr=dga_phys,\
                marker='o',markersize=10,mec='k',mfc='None',color='k',alpha=1,linestyle='None',\
                label=r'$g_A^{PDG}=%.4f(%s)$' %(ga_phys,str(dga_phys).split('0')[-1]))
            leg2.append(leg)
            ga_mpi_ax.vlines(epi_phys,0.5,1.6,linestyle='--',color='k')
