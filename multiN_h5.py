# Fit multiple smearings for Nucleon correlator for matrix prony NN
import numpy as np
import h5py as h5
import lsqfit
import gvar as gv
...
"""
the datasets are `sinks_{source}.mpi700.hdf5/pcorr/{0,1,2,3}`
0=point, 1=gauss 1.4, 2=gauss 2.8, 3=gauss 4.2.
And by `gauss14` in the filename above I really mean gauss 1.4. (edited)
"""
...
def read_file(params,fname,corr=None):
    if fname in ['sinks_point.mpi700.hdf5']:
        srckey = 'P'
        params['srckey'].append(srckey)
    elif fname in ['sinks_gauss.mpi700.hdf5','sinks_gauss14.mpi700.x0+100.hdf5']:
        srckey = 'S1p4'
        params['srckey'].append(srckey)
    if corr==None:
        corr = dict()
    else: pass
    snkkey = ['P', 'S1p4', 'S2p8', 'S4p2']
    pname = '/Users/cchang5/Physics/c51/data'
    f = h5.File('%s/%s' %(pname,fname))
    for i, k in enumerate(snkkey):
        corr['%s_%s' %(k,srckey)] = np.array(f['pcorr/%s' %i].value)[:,:,0]
        if srckey == 'P' and k in ['S1p4', 'S2p8', 'S4p2']:
            print k,srckey
            corr['%s_%s' %(k,srckey)] = corr['%s_%s' %(k,srckey)]/(2.*np.pi*float(k.split('S')[-1].replace('p','.')))**3
    f.close()
    return corr

def make_gvars(params,corr):
    data = []
    for k in params['corrs']:
        if len(data) == 0:
            data = corr[k]
        else:
            data = np.concatenate((data,corr[k]),axis=1)
...
def plot_data(params,data):
    ...
    for n in range(Ncorr):
        corr_dat = data[n*T:(n+1)*T]
        cdic[params['corrs'][n]] = corr_dat
        meff = np.array([np.arccosh((corr_dat[(t+1)%len(corr_dat)]+corr_dat[t-1])/(2*corr_dat[t])) for t in range(T)])
        mdic[params['corrs'][n]] = meff
        ax.errorbar(x=range(len(corr_dat)),y=[i.mean for i in meff],yerr=[i.sdev for i in meff],marker='s',fillstyle='none',label=params['corrs'][n])
    ax.legend()
    ax.set_ylim([0.5,1.5])
    ax.set_xlim([0,25])
...
if __name__=='__main__':
    params = meta()
    corr = read_file(params, 'sinks_gauss14.mpi700.x0+100.hdf5')
    #corr = read_file(params,'sinks_gauss.mpi700.hdf5')
    #corr = read_file(params,'sinks_point.mpi700.hdf5',corr)
